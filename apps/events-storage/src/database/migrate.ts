...
async function runMigrations() {
  await createDatabaseIfNotExists(config.postgres.eventsDatabase);
  ...
  const client = new Client({
    ...
    user: config.postgres.username,
    password: config.postgres.password,
    database: config.postgres.eventsDatabase,
  });

  try {
    await client.connect();
    logger.info(`Connected to ${config.postgres.eventsDatabase} database`);
